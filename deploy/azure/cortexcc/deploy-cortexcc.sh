...
# Config: deploy/azure/config/cortexcc.stg | cortexcc.prd
# Azure:  deploy/azure/azure.stg | azure.prd
#
# Uso: ./deploy/azure/cortexcc/deploy-cortexcc.sh [stg|prd]
...
ENV_FILE="${ENV_FILE:-$CONFIG_DIR/cortexcc.$DEPLOY_TARGET}"
...
load_env() {
  if [[ ! -f "$ENV_FILE" ]]; then
    ...
    log_info "Staging:  cp deploy/azure/config/cortexcc.stg.example deploy/azure/config/cortexcc.stg"
    log_info "Produccion: cp deploy/azure/config/cortexcc.prd.example deploy/azure/config/cortexcc.prd"
    exit 1
  fi
  set -a
  ...
  if [[ "$MANAGE_REDIS" == "true" ]]; then
    if [[ -z "${REDIS_NAME:-}" ]]; then
      log_err "Con MANAGE_REDIS=true debes definir REDIS_NAME en la config"
      exit 1
    fi
  elif [[ -z "${REDIS_URL:-}" ]]; then
    log_err "Con MANAGE_REDIS=false debes definir REDIS_URL en la config"
    exit 1
  fi
  ...
  for v in "${required[@]}"; do
    if [[ -z "${!v:-}" ]]; then
      log_err "Falta variable requerida en config: $v"
      exit 1
    fi
  done
  ...
  RUN_SETUP_MASTER="${RUN_SETUP_MASTER:-false}"
  BOOTSTRAP_TENANT="${BOOTSTRAP_TENANT:-false}"
  if [[ "$RUN_SETUP_MASTER" == "true" && "${RUN_SETUP_PLATFORM_ADMIN:-}" != "false" ]]; then
    RUN_SETUP_PLATFORM_ADMIN=true
  else
    RUN_SETUP_PLATFORM_ADMIN="${RUN_SETUP_PLATFORM_ADMIN:-false}"
  fi
  TENANT_SEED="${TENANT_SEED:-false}"
  TENANT_SKIP_DB_CREATE="${TENANT_SKIP_DB_CREATE:-false}"
  ...
  if [[ "$BOOTSTRAP_TENANT" == "true" ]]; then
    if [[ -z "${TENANT_KEY:-}" || -z "${TENANT_NAME:-}" ]]; then
  ...
  if [[ "$RUN_SETUP_PLATFORM_ADMIN" == "true" ]]; then
    if [[ -z "${PLATFORM_ADMIN_EMAIL:-}" || -z "${PLATFORM_ADMIN_PASSWORD:-}" ]]; then
      log_err "Con RUN_SETUP_PLATFORM_ADMIN=true define PLATFORM_ADMIN_EMAIL y PLATFORM_ADMIN_PASSWORD en la config"
      exit 1
    fi
    if [[ "$CORTEXCC_ENV" == "prd" && "$PLATFORM_ADMIN_PASSWORD" == "PlatformAdmin123!" ]]; then
      log_err "Cambia PLATFORM_ADMIN_PASSWORD en produccion (valor por defecto no permitido)"
      exit 1
    fi
  fi

  if [[ -z "${TENANT_CUSTOM_DOMAIN:-}" && -n "${FRONTEND_WEBAPP_NAME:-}" ]]; then
    TENANT_CUSTOM_DOMAIN="${FRONTEND_WEBAPP_NAME}.azurewebsites.net"
  fi
  TENANT_SUBDOMAIN="${TENANT_SUBDOMAIN:-${TENANT_KEY:-}}"

  if [[ -z "${TENANT_DB_NAME:-}" && -n "${DATABASE_URL:-}" ]]; then
    TENANT_DB_NAME="$(DATABASE_URL="$DATABASE_URL" node -e "
      const u = new URL(process.env.DATABASE_URL);
      console.log(u.pathname.replace(/^\//, '').split('?')[0]);
    " 2>/dev/null || true)"
    if [[ -n "${TENANT_DB_NAME:-}" ]]; then
      TENANT_SKIP_DB_CREATE="${TENANT_SKIP_DB_CREATE:-true}"
    fi
  fi
  ...
  if [[ "$CORTEXCC_ENV" == "prd" ]]; then
    if [[ -z "${JWT_SECRET:-}" ]]; then
      log_err "En produccion (CORTEXCC_ENV=prd) debes definir JWT_SECRET en $ENV_FILE"
      exit 1
    fi
    if [[ -z "${PLATFORM_JWT_SECRET:-}" ]]; then
      PLATFORM_JWT_SECRET="${JWT_SECRET}-platform-admin"
      log_warn "PLATFORM_JWT_SECRET no definido; usando derivado de JWT_SECRET (recomendado: secreto propio en prd)"
    fi
  else
    if [[ -z "${JWT_SECRET:-}" ]]; then
...
ensure_redis_cache() {
  if [[ "$MANAGE_REDIS" != "true" ]]; then
    log_info "MANAGE_REDIS=false; se usa REDIS_URL de la config"
    return 0
  fi
...
bootstrap_master_and_tenant() {
  if [[ "$RUN_SETUP_MASTER" != "true" && "$BOOTSTRAP_TENANT" != "true" && "$RUN_SETUP_PLATFORM_ADMIN" != "true" ]]; then
    return 0
  fi

  log_step "Bootstrap Master DB, admin de plataforma y tenant inicial"
  require_command node
  require_command npm

  if [[ ! -d "$BACKEND_DIR/node_modules" ]]; then
    log_info "Instalando dependencias backend (npm ci)..."
    (cd "$BACKEND_DIR" && npm ci)
  fi

  (
    cd "$BACKEND_DIR"
    export MASTER_DATABASE_URL DATABASE_URL

    log_info "Generando clientes Prisma..."
    npm run prisma:generate

    if [[ "$RUN_SETUP_MASTER" == "true" ]]; then
      log_info "Ejecutando setup:master..."
      npm run setup:master
      log_ok "Master DB inicializada (cortexcc_master + tenants + platform_admins)"
    fi

    if [[ "$RUN_SETUP_PLATFORM_ADMIN" == "true" ]]; then
      log_info "Ejecutando setup:platform-admin (${PLATFORM_ADMIN_EMAIL})..."
      export PLATFORM_ADMIN_EMAIL PLATFORM_ADMIN_PASSWORD
      export PLATFORM_ADMIN_FIRST_NAME="${PLATFORM_ADMIN_FIRST_NAME:-Platform}"
      export PLATFORM_ADMIN_LAST_NAME="${PLATFORM_ADMIN_LAST_NAME:-Admin}"
      npm run setup:platform-admin
      log_ok "Admin de plataforma listo: ${FRONTEND_URL}/platform/login"
    fi
...
main() {
  ...
  load_env
  echo " Entorno: $CORTEXCC_ENV ($DEPLOY_TARGET)"
  echo ""
  check_dependencies
  validate_local_project_ports
  ensure_azure_session
  ...
  if [[ "$RUN_SETUP_PLATFORM_ADMIN" == "true" || "$RUN_SETUP_MASTER" == "true" ]]; then
    echo "Platform: ${FRONTEND_URL}/platform/login"
  fi
  if [[ "$MANAGE_REDIS" == "true" ]]; then
